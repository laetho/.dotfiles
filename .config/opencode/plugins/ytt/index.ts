...
function isPrivateIP(ip: string): boolean {
  ...
  return (
    ipv6Lower.startsWith("fc") || // ULA: fc00::/7
    ipv6Lower.startsWith("fd") || // ULA: fc00::/7
    ipv6Lower.startsWith("fe80") || // link-local: fe80::/10
    ipv6Lower.startsWith("fec0") || // site-local (deprecated): fec0::/10
    ipv6Lower === "::1" || // loopback
    ipv6Lower === "::" || // unspecified
    ipv6Lower.startsWith("::ffff:") // IPv4-mapped
  )
}

// Valid language codes (basic 2-letter codes only for security)
...
/**
 * Represents a single text segment in the transcript with timing and confidence info
 */
interface TranscriptSegment {
  /** The text content of this segment */
  utf8: string
  /** Offset time in milliseconds from the start of the event */
  tOffsetMs?: number
  /** ASR confidence score (0-1) for automatic speech recognition */
  acAsrConf?: number
}

/**
 * Represents a time-delimited event containing one or more text segments
 */
interface TranscriptEvent {
  ...
  dDurationMs?: number
  /** Window ID for display positioning */
  wWinId?: number
  /** Whether this event appends to previous content */
  aAppend?: number
  /** Array of text segments in this event */
  segs?: TranscriptSegment[]
}

/**
 * Root transcript data structure from yt-dlp's JSON3 format
 */
...
const YoutubeTranscriptPlugin: Plugin = async (ctx) => {
  return {
    tool: {
      ytt: tool({
        ...
        async execute(args, context): Promise<string> {
          ...
          let videoId: string = trimmedUrl

            if (trimmedUrl.includes("://")) {
             try {
               const parsed = new URL(trimmedUrl)

               // Only allow https scheme (YouTube videos are always served over HTTPS)
               ...
               // Handle /watch?v=VIDEO_ID format (possibly with &t= parameter)
               if (parsedPath === "/watch") {
               ...
               // Handle youtu.be short URLs (possibly with &t= parameter)
            ...
            // Validate video ID format (YouTube video IDs are 11 alphanumeric characters)
           ...
           try {
             // Check if yt-dlp is available with a 5-second timeout
             ...
             if (ytDlpCheck.status !== 0) {
               throw new Error(ERRORS.YT_DLP_NOT_FOUND)
                ...
                // Create a temporary directory for the download with random prefix for security
                // Use crypto.randomBytes for strong randomness to prevent symlink attacks
                const randomSuffix = crypto.randomBytes(16).toString("hex")
                const tempDir = fs.mkdtempSync(
                  path.join("/tmp", "ytt-" + randomSuffix + "-")
                )
             ...
             try {
                ...
                // Validate language code against whitelist to prevent injection
                // Only allow base language codes (e.g., "en", "pt") for security
                const langCodeBase = langCode.split("-")[0]
                if (!VALID_LANG_CODES.has(langCodeBase)) {
                  throw new Error(ERRORS.INVALID_LANG)
                }
                
                // Only allow language codes matching the pattern ^[a-z]{2}(-[A-Z]{2,})?$
                // This prevents command injection via special characters
                if (!/^[a-z]{2}(-[A-Z]{2,})?$/.test(langCode)) {
                  throw new Error(ERRORS.INVALID_LANG)
                }

                // Use yt-dlp to download the subtitle file with a 30-second timeout
                // Pass arguments as separate array elements to prevent command injection
                const ytDlpProcess = spawnSync(
                  ...
                  [
                    ...
                    "--sub-format=json3",
                    "--output",
                    path.join(tempDir, "transcript.json3"),
               ...
               if (ytDlpProcess.status !== 0) {
                 ...
                 // Any other error from yt-dlp indicates a failure
               ...
               if (!json3File) {
                ...
                const filePath = path.resolve(path.join(tempDir, json3File))
                const resolvedTempDir = path.resolve(tempDir)
                
                // Additional validation: check that the file name is safe
                // Only allow alphanumeric, hyphens, underscores, and the .json3 extension
                if (!/^[a-zA-Z0-9._-]+\.json3$/.test(json3File)) {
                  throw new Error(ERRORS.NO_TEMP_FILE)
                }
                
                // Verify the path is within the expected temp directory
                const normalizedFilePath = path.normalize(filePath)
                const normalizedTempDir = path.normalize(resolvedTempDir)
                if (
                ...
                // Validate video ID for use in URL - ensure it's safe for URL construction
                // Already validated as 11 alphanumeric characters, but double-check
                if (!/^[A-Za-z0-9]{11}$/.test(videoId)) {
                  throw new Error(ERRORS.INVALID_VIDEO_ID)
                }
              ...
              if (!transcriptText || transcriptText.length === 0) {
               ...
               return `\`\`\`markdown\n${transcriptText}\n\`\`\``
                ...
                try {
                  ...
                  // Log cleanup failures for debugging but don't block the main flow
                  // This could happen on systems with strict temp directory quotas
                  console.warn("Warning: Failed to clean up temporary directory:", (cleanupError as Error).message)
                }
              }
           } catch (error: unknown) {
             if (error instanceof Error) {
               ...
               // Check for subtitle-related error messages
...
export default YoutubeTranscriptPlugin
